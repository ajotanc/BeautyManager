<template>
  ...
  >
    <Fluid v-if="visible">
      <Form
        id="customer-form-element"
        ref="formRef"
        :key="props.customerToEdit?.$id || 'new'"
          ...
          <FormField name="phone" v-slot="$field" class="flex flex-col gap-1">
            <FloatLabel variant="in">
              <InputText
                ...
                :invalid="$field?.invalid"
              />
              <label for="cust_phone">WhatsApp / Celular *</label>
            </FloatLabel>
            <Message v-if="$field?.invalid" severity="error" size="small" variant="simple">
              {{ $field.error?.message }}
            </Message>
          ...
          <FormField name="birth_date" v-slot="$field" class="flex flex-col gap-1">
            <FloatLabel variant="in">
              <InputText
                ...
                :invalid="$field?.invalid"
              />
              <label for="cust_birth">Data de Aniversário (DD/MM)</label>
            </FloatLabel>
            <Message v-if="$field?.invalid" severity="error" size="small" variant="simple">
              {{ $field.error?.message }}
            </Message>
        ...
        <FormField name="notes" v-slot="$field" class="flex flex-col gap-1">
          <FloatLabel variant="in">
            <Textarea
              ...
              autoResize
              fluid
              placeholder="Preferências de cosméticos, perfumes favoritos, tom de base..."
            />
...
const initialValues = computed<CustomerFormData>(() => {
  return {
    ...
    birth_date: props.customerToEdit?.birth_date || '',
